...
window.saveFood = async () => {
    ...
    if (!name || calories === 0) {
        showNotification('Please enter food name and calories', 'error');
        return;
    }
    ...
    try {
        const foodEntry = {
            ...
            servingSize: document.getElementById('food-serving').value,
            date: serverTimestamp(),
            createdAt: serverTimestamp()
        };
        ...
        if (currentFoodPhoto) {
            ...
            // const photoUrl = await uploadFoodPhoto(currentFoodPhoto);
            // foodEntry.photoUrl = photoUrl;
...
window.quickAddFood = async (catalogId) => {
    try {
        ...
        if (catalogDoc.exists()) {
            ...
            const mealType = prompt('Select meal type: breakfast, lunch, dinner, or snack', 'lunch');
            ...
            await addDoc(collection(db, 'users', currentUser.uid, 'foodEntries'), foodEntry);
            showNotification('Food added to today\'s meals!', 'success');
...
// Edit food entry (simplified - you can expand this)
...
// Save weight from dashboard (linked with workout section)
window.saveDashboardWeight = async () => {
    ...
    try {
        // Save to same measurements collection as workout section
        await addDoc(collection(db, 'users', currentUser.uid, 'measurements'), {
            userId: currentUser.uid,
            weight: val,
            chest: null,
        ...
        // Also update workout section weight chart if needed
        if (window.loadWeightChart) {
            await loadWeightChart(currentWeightRange);
        }
...
// Load weight chart for dashboard (uses same data as workout section)
async function loadDashboardWeightChart() {
    ...
    try {
        ...
        if (dashboardWeightChart) dashboardWeightChart.destroy();
        
        const ctx = canvas.getContext('2d');
        const ChartClass = window.Chart || Chart;
        
        dashboardWeightChart = new ChartClass(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
        ...
        console.error('Error loading dashboard weight chart:', err);
    }
}
